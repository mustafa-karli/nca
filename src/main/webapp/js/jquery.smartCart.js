...
;(function ($, window, document, undefined) {
    ...
    var defaults = {
        ...
        cartItemTemplate: '<div class="col-md-1 col-sm-1 padding5"><button type="button" class="pull-right sc-cart-remove btn btn-danger btn-xs fa fa-trash-o"></button></div><div class="col-md-8 col-sm-8"><h6 class="list-group-item-heading cart_product_caption">{product_name}</h6></div>',
        cartItemQtyTemplate: '{display_quantity}',
//a        cartItemQtyTemplate: '{display_price} × {display_quantity} = {display_amount}',
        ...
        lang: { // Language variables
        	cartTitle: "SİPARİŞ SEPETİ",
            checkout: 'Onay',
            clear: 'Temizle',
            addToFav: 'Favorilere Ekle',
            subtotal: 'Toplam Ürün:',
            cartRemove: '×',
            cartEmpty: 'Sipariş sepetiniz boş!'
//            	cartTitle: "${SHOPPING_CART}",
//                checkout: '${APPROVE}',
//                clear: '${CLEAR}',
//                subtotal: '${SUBTOTAL}:',
//                cartRemove: '×',
//                cartEmpty: '${CART_EMPTY}'
        ...
        toolbarSettings: {
            ...
            showFavCheck: true, //a for favorites
    ...
    $.extend(SmartCart.prototype, {
        ...
        _setElements: function () {
            ...
            this.cart_element.append('<div class="list-group sc-cart-item-list" style="height:80%; overflow-y: scroll;"></div>');
        },
        /* 
        ...
        _setToolbar: function () {
            ...
            //a Add to favorite checkbox
            if (this.options.toolbarSettings.showFavCheck) {
                var chkFavorite = $('<div class="checkbox"><label><input class="" type="checkbox" name="addToFav" id="addToFav"/>' + this.options.lang.addToFav + '</label></div>');
                toolbarButtonPanel.append(chkFavorite);
            }
        ...
        _setEvents: function () {
            ...
            $(this.options.addCartSelector).on("click", function (e) {
                ...
                p = mi._addToCart(p);
   //a             $(this.button).parents(mi.options.productContainerSelector).addClass('sc-added-item').attr('data-product-unique-key', p.unique_key);
                $(this).addClass('sc-added-item').attr('data-product-unique-key', p.unique_key);
        ...
        _getProductDetails: function (elm) {
        	...
        	var mi = this;
            var p = {};
  //a          elm.parents(this.options.productContainerSelector).find(this.options.productElementSelector).each(function () {
            elm.find(this.options.productElementSelector).each(function () {       	
        ...
        _addUpdateCartItem: function (p) {
            ...
            if (elmMain && elmMain.length > 0) {
                ...
                elmMain = $('<div></div>').addClass('sc-cart-item list-group-item row cart_margin');
                //elmMain.append('<button type="button" class="pull-right sc-cart-remove btn btn-danger btn-xs fa-delete">' + this.options.lang.cartRemove + '</button>');
...
/*                var itemSummary = '<div class="sc-cart-item-summary">';
                itemSummary += '<input type="number" min="1" max="10000" class="sc-cart-item-qty form-control" value="' + this._getValueOrEmpty(p[this.options.paramSettings.productQuantity]) + '" />';
                itemSummary += '<button type="button" class="pull-right sc-cart-remove btn btn-danger btn-xs fa-delete">' + this.options.lang.cartRemove + '</button>';
                itemSummary += '</div>';
*/
                var itemSummary = ''; //<div class="row">';
                	itemSummary += '<div class="col-md-3 col-sm-3"><input type="number" min="1" max="1000" class="sc-cart-item-qty form-control" value="' + this._getValueOrEmpty(p[this.options.paramSettings.productQuantity]) + '"></div>';
                	//itemSummary += '</div>';
        ...
        _hasCartChange: function () {
            ...
            if (this.cart.length == 0 && this.options.cart.length == 0) {
...
//a					subtotal += (p[mi.options.paramSettings.productPrice] - 0) * (p[mi.options.paramSettings.productQuantity] - 0);
                    subtotal += (p[mi.options.paramSettings.productQuantity] - 0);
                    
                }
            });
//a            return this._getMoneyFormatted(subtotal);
            return subtotal;
        },
        /* 
        ...
        _submitCart: function () {
            ...
            switch (this.options.submitSettings.submitType) {
                ...
                default:
                	...
                	doAjaxPost('materialRequest/entry');
                    //formElm.submit();
                    this._triggerEvent("cartSubmitted", [this.cart]);
